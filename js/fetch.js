async function fetchData() {
    ...
    const data_url = "https://statfin.stat.fi/PxWeb/api/v1/en/StatFin/kvaa/km_ku.px";
    ...
    const res2 = await fetch(coordinates_url);
    const data2 = await res2.json();

    data2.features.sort((a, b) => a.properties.nimi.localeCompare(b.properties.nimi));
    ...
    var map = L.map("map", { minZoom: -3 }).setView([51.505, -0.09], 13);;
...
function printData(party,municipality,values) {
  const head = document.createElement("tr");
  //console.log(party.textContent);

  const municipalityhead = document.createElement("th");
  municipalityhead.textContent = "Municipality";
  head.appendChild(municipalityhead);

  const partyhead = document.createElement("th");
  partyhead.textContent = party.textContent;
  head.appendChild(partyhead);

  const partyStyle = getPartyColor(party.textContent);
  Object.assign(head.style, partyStyle);

  const table_head = document.getElementById("data_table_head");
  table_head.innerHTML = "";
  table_head.appendChild(head);

  var n=0;

  switch (party.textContent) {
    case "Keskusta":
      n=2;
      break;
  ...
  for (let i = 0; i < municipality.length; i++) {
    ...
    const row_municipality = document.createElement("td");
